...
int main(void)
{
	...
	//GLuint Texture = loadBMP_custom("carTexture2.bmp");
	...
	bool res = loadOBJ("untitledCar.obj", vertices, uvs, normals);
	...
	//--------------------------------additions-------------------------------------------------
	float carSpeed = 0.07f;
	float moveUnit = 0.0f;
	float enemyPosX = 2.0f;
	...
	do {
		...
		// Strafe right
		if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS && playerPos < 2.0f) {
			playerPos += 0.05f;
		}
		// Strafe left
		if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS && playerPos > -2.0f) {
			playerPos -= 0.05f;
		}
		//---------------------------------------end additions---------------------------------------
		...
		ModelMatrix1 = glm::translate(ModelMatrix1, glm::vec3(playerPos, -1.0f, playerPosZ));
		...
		//pull player towards the middle
		if (playerPos < 0) {
			playerPos += 0.01f;
		}
		if (playerPos > 0) {
		...
		ModelMatrix2 = glm::translate(ModelMatrix2, glm::vec3(enemyPosX, -1.0f, moveUnit));
		glm::mat4 MVP2 = ProjectionMatrix * ViewMatrix * ModelMatrix2;


		//-----------------------additions-----------------------------------------------
		// move enemy, if past camera, "spawn" in the back in between positions -2, 0 or 2
		if (score > 0) {
			...
			if (moveUnit < playerPosZ + 2.5f && moveUnit > playerPosZ - 2.5f && enemyPosX <= playerPos + 1.5f && enemyPosX >= playerPos - 1.5f) {
				score -= 0.1f;
			}
		}
		...
		if (score > 0) {
			...
			sprintf(scoreText, " %.0f points", score);
			printText2D(scoreText, 50, 550, 20);
			timeAlive = glfwGetTime();
		}
		else {
			sprintf(scoreText, "%.2f sec", timeAlive);
			printText2D(scoreText, 50, 550, 20);
		}
